...
def _getDataRangeNew(dataCache, i, n):
    '''
    returns [i,n)
    '''
    res = np.zeros(n-i)
...
def _lazyLoadFindEventsNew(**parameters):
    event_count = 0
...
#                                 # lowers the samples/s rate.
        ...
        if 2000 < i < 2010:
            print datapoint, local_mean - threshold_start, local_mean + threshold_start
        if isEvent:
            isEvent = False
...
#                                 # lowers the samples/s rate.
            ...
            if done and event_end - event_start > min_event_steps:
                ...
                for q in range(0, n_levels):
                    ...
                    level_values[q] = np.mean(_getDataRangeNew(dataCache, start_index, end_index))
                for j, level_index in enumerate(level_indexes):
                    level_indexes[j] = level_index + placeInData
                # end CUSUM
                event = {}
                event['event_data'] = _getDataRangeNew(dataCache, event_start, event_end)
                event['raw_data'] = _getDataRangeNew(dataCache, event_start-raw_points_per_side, event_end+raw_points_per_side)
...
def findEvents(**parameters):
    ...
    return _lazyLoadFindEventsNew(**params)
