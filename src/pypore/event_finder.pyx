...
cdef _lazy_load_find_events(AbstractReader reader, Parameters parameters, object pipe=None, h5file=None, save_file_name=None):
    ...
    baseline_type.update_baseline(np.mean(data[0:initialization_index]))
    baseline_type.update_variance(np.var(data[0:initialization_index]))
    ...
    cdef:
        ...
        long ko = i
        double event_area = data_point - baseline_type.get_baseline()  # integrate the area
        ...
        double temp = 0
        long temp_long = 0
        double baseline = baseline_type.get_baseline()
        double variance = baseline_type.get_variance()
    ...
    while i < n:
        ...
        baseline = baseline_type.compute_baseline(data_point)
        variance = baseline_type.compute_variance(data_point)
        i += 1
